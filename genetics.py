...
def fitness_for_queue(core, queue):
    ...
    for task in queue:
        ...
        task_end_time = (task.start_time + exec_time)
        delta = task.deadline - task_end_time
        if delta >= 0:
            score += delta
        else:   # deadline missed
            # score += 10 * min(delta, -100)
            score -= 10 * max(-delta, miss_penalty)
            missed += 1
...
def grade(tasks, population):
    """
    Find min, average and max fitness for a population.
    """
    score = 0
    missed = 0
    for individual in population:
...
def main():
    ...
    # fitness_history = [grade(tasks, population)[0], ]
    for i in range(tot_generations):
        population = evolve(tasks, population, fitness)
        fitness, misses = get_population_fitness(tasks, population)
        
        fitness_max_history.append( max(fitness) )
        fitness_min_history.append( min(fitness) )
        fitness_mean_history.append( sum(fitness)/len(fitness) )
        score = max(fitness)
        missed = sum(misses)/len(misses)
        # score, missed = grade(tasks, population)
        # fitness_history.append(score)
        print('iteration {} population: {} max_score: {} avg_missed: {}'.format(i + 1, population_size, score, missed))
